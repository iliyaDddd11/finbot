...
def generate_text_section(data: Dict, prompt_type: str, api_key: str, company_name: str, company_ticker: str, base_url: str = None, model: str = None) -> str:
    ...
    # Call LLM API with exponential back-off retry (handles 429 / transient errors)
    _max_retries = 3
    _base_delay  = 2.0
    ...
    for attempt in range(1, _max_retries + 1):
        try:
            ...
            last_exc = e
            # Rate-limit or server error — retry with back-off
            err_str = str(e).lower()
            is_retryable = any(code in err_str for code in ("429", "rate", "502", "503", "504", "timeout"))
            if is_retryable and attempt < _max_retries:
                wait = _base_delay * (2 ** (attempt - 1))
                print(f"⚠️ LLM error on attempt {attempt}/{_max_retries} for '{prompt_type}': {e}; "
                      f"retrying in {wait:.0f}s …")
                time.sleep(wait)
            else:
                break

    print(f"❌ Error generating '{prompt_type}' after {_max_retries} attempts: {last_exc}")
    return _get_fallback_text(prompt_type, company_name)
