...
def calculate_growth_and_forecasts(df_historical: pd.DataFrame, forecast_config: dict) -> pd.DataFrame:
    ...
    # FIX: EBITDA must be derived from EBITDA Margin × Revenue, NOT from
    # Contribution Profit − SG&A (those are different accounting concepts and
    # the naive subtraction was inflating EBITDA Margin by 700+ bps).
    for f_year in forecast_years:
        rev = df_final.loc[df_final['metrics'] == 'Revenue', f_year].values[0]
        sga = df_final.loc[df_final['metrics'] == 'SG&A', f_year].values[0]

        # Contribution margin → Contribution Profit / Cost of Ops
        ...
        # EBITDA = Revenue × EBITDA Margin  (margin was set in the margin_improvement loop above)
        ebitda_margin_str = str(df_final.loc[df_final['metrics'] == 'EBITDA Margin', f_year].values[0])
        if pd.notna(rev) and '%' in ebitda_margin_str:
            ebitda_margin = float(ebitda_margin_str.replace('%', '')) / 100
            ebitda = rev * ebitda_margin
            df_final.loc[df_final['metrics'] == 'EBITDA', f_year] = round(ebitda, 2)
            # EBITDA Margin row is already set correctly — no override needed
