...
def sanitize_api_value(value: str | None) -> str:
    """Normalizes API values copied from dashboards or curl examples."""
    if value is None:
        return ""
    value = str(value).strip().strip('"').strip("'")
    value = re.sub(r"^\?apikey=", "", value, flags=re.IGNORECASE)
    return value.strip()
...
def get_api_key(config, section="API_KEYS", key="fmp_api_key"):
    """Retrieves and sanitizes a specific API key.

    Priority order:
      1. Environment variable (set by web app for per-request keys)
      2. config.ini entry
    """
    env_var = _ENV_KEY_MAP.get(key)
    if env_var:
        env_val = sanitize_api_value(os.environ.get(env_var, ""))
        if env_val:
            return env_val
    try:
        return sanitize_api_value(config.get(section, key))
